...
static const TIntermTyped* findLValueBase(const TIntermTyped* node, pbr::Symbol& symbol)
{
    do {
        ...
        if (op == EOpIndexDirectStruct) {
            ...
            access.type = pbr::Access::DirectIndexForStruct;
        } else {
            access.string = op2Str(op) ;
            access.type = pbr::Access::Swizzling;
        }
        symbol.add(access);
...
public:
    explicit SymbolsTracer(std::deque<pbr::Symbol>& events) : mEvents(events) {
    }
    ...
    bool visitAggregate(TVisit, TIntermAggregate* node) override {
        ...
        for (size_t parameterIdx = 0; parameterIdx < node->getSequence().size(); parameterIdx++) {
            ...
            if (parameter->getAsBinaryNode()) {
                ...
                for (pbr::Symbol symbol: events) {
                    pbr::Access fCall = {pbr::Access::FunctionCall, functionName, parameterIdx};
                    symbol.add(fCall);
            ...
            if (parameter->getAsSymbolNode()) {
                pbr::Symbol s(parameter->getAsSymbolNode()->getName().c_str());
                pbr::Access fCall = {pbr::Access::FunctionCall, functionName, parameterIdx};
                s.add(fCall);
                mEvents.push_back(s);
            }
...
public:
    ...
    void visitSymbol(TIntermSymbol* node) override {
        pad();
        std::cerr << "pbr::Symbol " << node->getAsSymbolNode()->getName().c_str();
        std::cerr << std::endl;
    }
...
void traceSymbols(TIntermNode& functionNode, std::deque<pbr::Symbol>& events) {
